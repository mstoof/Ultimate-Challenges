...
import { events, rsvps, gearSports } from "@/db/schema";
...
export async function createEvent(formData: FormData) {
  ...
  if (formData.get("returnTo") === "gear") {
    await db.update(gearSports)
      .set({ targetEventId: created.id })
      .where(and(eq(gearSports.userId, session.user.id), eq(gearSports.sport, created.sport)));
    redirect("/gear");
  }
