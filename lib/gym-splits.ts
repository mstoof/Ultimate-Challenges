...
export function gymInstructions(gymDays: number, input: unknown): string {
  ...
  const focus = splits.length
    ? `Gekozen volledige gymsplit${splits.length > 1 ? "s" : ""}: ${splits.map((split) => `${split.label} (${split.focus}; ${split.days})`).join("; ")}. ` +
      'Gebruik één gekozen systeem als leidraad: zet de fase of trainingsdag van de split in de gymsessietitel (bijvoorbeeld “PPL — Push” of “Upper/Lower — Lower”) en kies oefeningen passend bij die fase. Wissel de fases in vaste volgorde af over de beschikbare gymdagen en weken. Meerdere opties voegen geen extra gymdagen toe. '
